...
ObjectsTable objectsArray[1000];//Массив для храненения записей таблицы ObjectsTable
int objectCount = 0;// Количество записей в таблице

					//----- Чтение всего массива записей из открытого файла
...
//----- Сортировка таблицыы ObjectsTable
//----- Выполняем сортировку по полям CountryObjects и NameObject
void sort_objects(int NumberObjects)
{
	if (NumberObjects > 1)// сортировать если число записей больше 1
	{
		for (int j = 0; j< NumberObjects; j++)
		{
			for (int i = 0; i < NumberObjects - 1; i++)
			{
				// сортируем в алфатином порядке по странам и объектам
...
//парсинг по начальному фрагменту имени объекта и стране
//Записывает в массив структур resultCount найденые совпадения для дальнейшего вычисления
int Parser(char *NamePattern, char *country, ObjectsTable *resultCount)
{
	...
	for (int i = 0; i < objectCount; i++)
	{
		...
		if (res_country)
		{
			...
			//запись найденого объекта в массив
			if (res_pattern)
			{
